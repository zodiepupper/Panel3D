...
@export var vis: InteractionRayVisualizer
...
## if this is enabled, the panel will revieve any input event coming through the window
## when that panel is focused (based on which one you're pointing at rn)
@export var forward_all_mouse_buttons : bool = false
...
func _input(event):
	...
	if forward_all_mouse_buttons:
		if is_colliding() and "laser_input" in prevHover and event is InputEventMouseButton:
			prevHover.laser_input({
				'action':'custom',
	...
	if follow_mouse and event is InputEventMouse:
		...
		if event is InputEventMouseButton and event.pressed and !forward_all_mouse_buttons:
...
func grab(target:Node=null):
	if !is_instance_valid(target) and is_colliding():
		target = get_collider()
	if target is Node3D:
		grabbed_nodes[target.get_instance_id()] = GrabbedNode3D.create_from_target(target)
		return
	if target is Node2D:
		pass
